...
class MedicalRecord(models.Model):
    ...
    # doctor who provided this record (useful for second opinions)
...
class ShareableLink(models.Model):
    """Model to store unique tokens for sharing a user's health profile"""
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='shareable_link')
    token = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    created_at = models.DateTimeField(auto_now_add=True)
    
    def __str__(self):
        return f"Shareable link for {self.user.username}"
